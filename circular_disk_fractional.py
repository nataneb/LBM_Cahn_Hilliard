...
#
#This program is the realization of the test case of the stationary circular disk via the LBM
#for time-fractional Cahn-Hilliard equation provided by  Liang et al. (2020)
#
# Static circular disk
#
...
prefix3        = 'x_axis'
outputFolder  = './out_circular_disk_fractional_CH_different_alphas' #Folder where all results are saved
...
# Coordinates of the center of the disk and its radius length in lattice units
cx = nx*0.5
cy = ny*0.5
r  = 25
...
#defining the exact shape of the disk
...
for i in range(size(alpha)):
    ...
    for time in range(maxIterations):
        ...
        #Incorporating forcing distribution function
        fpost = fpost + F_is *dt
        #Streaming step
        fin = stream2(fpost)
...
#Drawing the shape of the disk based on the numerical results along the analytical solution for different fractional orders
