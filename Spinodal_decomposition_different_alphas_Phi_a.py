...
#
#This program is the realization of the spinodal decomposition test case via the LBM
...
outputFolder  = './out_spinodal_decomposition_different_alphas_different_phi_as'    #Folder where all results are saved
workingFolder = os.getcwd()

###### Setup of the experiment #######
maxIterations = 18000    # Total number of iterations in time
...
#interfacial thickness, surface tensions and the parameters defined by them
D=3
sigma =0.1
kappa = (3./8.)*sigma*D
beta =  (3./4.)*sigma/D
...
#Proportion of liquid and gas (negative means more air)
phi_a = array([-0.3, 0.1])
...
for a in range(size(phi_a)):
    ...
    energies[a,2,0] = energies[a,0,0]
    
    #Lopp over fractional orders
        ...
        for time in range(maxIterations):
            ...
            if ( ((time+1) % plotEveryN == 0) & (liveUpdate | saveVTK | savePlot) & (time > skipFirstN) ):
                if ( liveUpdate | savePlot ):
                    ...
                    axfi.set_title("Order parameter")
                    pyplot.colorbar(im1, ax=axfi,orientation='horizontal')
                    pyplot.savefig(prefix2 + "_alpha ="+ str(alpha[i]) + "_" + "phi_a =" +str(phi_a[a])+"_" + str(time +1).zfill(4) + ".png", dpi=200)
                    pyplot.close()   
            #Calculating energy
...
pyplot.plot(t, energies[0,0,:], 'k')
pyplot.plot(t, energies[0,1,:], 'r')
pyplot.plot(t, energies[0,2,:], 'b')
pyplot.plot(t, energies[1,0,:], 'k--')
pyplot.plot(t, energies[1,1,:], 'r--')
pyplot.plot(t, energies[1,2,:], 'b--')
pyplot.legend([r'$\alpha = $'+str(alpha[0])+', '+r'$\phi_a = $'+str(phi_a[0]), r'$\alpha = $'+str(alpha[1])+', '+r'$\phi_a = $'+str(phi_a[0]), r'$\alpha = $'+str(alpha[2])+', '+r'$\phi_a = $'+str(phi_a[0]), r'$\alpha = $'+str(alpha[0])+', '+r'$\phi_a = $'+str(phi_a[1]), r'$\alpha = $'+str(alpha[1])+', '+r'$\phi_a = $'+str(phi_a[1]), r'$\alpha = $'+str(alpha[2])+', '+r'$\phi_a = $'+str(phi_a[1])])
pyplot.xlim(0,21000)
pyplot.ylim(40,260)
pyplot.xlabel('t')
pyplot.ylabel('E')
